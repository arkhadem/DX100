...
int create_relation_pk(relation_t *relation, int32_t num_tuples) {
    check_seed();

    relation->num_tuples = num_tuples;
    relation->tuples = (tuple_t *)MALLOC(relation->num_tuples * sizeof(tuple_t));
    relation->tuples_start = relation->tuples;
    relation->tuples_end = relation->tuples + (relation->num_tuples + RELATION_PADDING / sizeof(tuple_t));
    memset(relation->tuples, 0, relation->num_tuples * sizeof(tuple_t) + RELATION_PADDING);
...
int parallel_create_relation_pk(relation_t *relation, int32_t num_tuples,
    ...
    memset(relation->tuples, 0, num_tuples * sizeof(tuple_t) + RELATION_PADDING);
...
#pragma omp parallel
    {
        ...
        random_unique_gen_thread(&args[tid]);
    }
...
#pragma omp parallel
    {
        int tid = omp_get_thread_num();
        numa_localize_thread(&args[tid]);
    }

    return 0;
}

int create_relation_fk(relation_t *relation, int32_t num_tuples, const int32_t maxid) {
...
int create_relation_fk_from_pk(relation_t *fkrel, relation_t *pkrel,
    ...
    fkrel->tuples_start = fkrel->tuples;
    fkrel->tuples_end = fkrel->tuples + (num_tuples + RELATION_PADDING / sizeof(tuple_t));

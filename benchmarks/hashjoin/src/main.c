...
void save_relation_to_binary_file(relation_t *rel, const char *filename) {
    ...
    fwrite(&rel->num_tuples, sizeof(rel->num_tuples), 1, fp);

    /* Write the tuples */
    size_t write_count = fwrite(rel->tuples, sizeof(tuple_t), rel->num_tuples, fp);
    if (write_count != rel->num_tuples) {
        fprintf(stderr, "Error writing tuples to file %s.\n", filename);
        exit(EXIT_FAILURE);
    }
...
void load_relation_from_binary_file(relation_t *rel, const char *filename) {
    ...
    fread(&rel->num_tuples, sizeof(rel->num_tuples), 1, fp);

    /* Allocate memory for tuples */
    rel->tuples = (tuple_t *)MALLOC(rel->num_tuples * sizeof(tuple_t));
    rel->tuples_start = rel->tuples;
    rel->tuples_end = rel->tuples + (rel->num_tuples + RELATION_PADDING / sizeof(tuple_t));
    if (!rel->tuples) {
        fprintf(stderr, "Could not allocate memory for tuples.\n");
        exit(EXIT_FAILURE);
    }
    memset(rel->tuples, 0, rel->num_tuples * sizeof(tuple_t) + RELATION_PADDING);

    /* Read the tuples */
    size_t read_count = fread(rel->tuples, sizeof(tuple_t), rel->num_tuples, fp);
    if (read_count != rel->num_tuples) {
        fprintf(stderr, "Error reading tuples from file %s.\n", filename);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    numalocalize = 0; // cmd_params.basic_numa;
    ...
    /* Load relR and relS from binary files */
    char relS_name[128];
    char relR_name[128];

    if (cmd_params.s_size >= 1000000) {
        snprintf(relS_name, 128, "relS_%dM.dat", cmd_params.s_size / 1000000);
    } else if (cmd_params.s_size >= 1000) {
        snprintf(relS_name, 128, "relS_%dK.dat", cmd_params.s_size / 1000);
    } else {
        snprintf(relS_name, 128, "relS_%d.dat", cmd_params.s_size);
    }

    if (cmd_params.r_size >= 1000000) {
        snprintf(relR_name, 128, "relR_%dM.dat", cmd_params.r_size / 1000000);
    } else if (cmd_params.r_size >= 1000) {
        snprintf(relR_name, 128, "relR_%dK.dat", cmd_params.r_size / 1000);
    } else {
        snprintf(relR_name, 128, "relR_%d.dat", cmd_params.r_size);
    }

    fprintf(stdout, "[INFO ] Loading relation R from file '%s' : ", relR_name);
    fflush(stdout);
    load_relation_from_binary_file(&relR, relR_name);
    printf("OK \n");

    fprintf(stdout, "[INFO ] Loading relation S from file '%s' : ", relS_name);
    fflush(stdout);
    load_relation_from_binary_file(&relS, relS_name);
    printf("OK \n");

#endif /* LOAD */

#ifdef DUMP
    /* Save relR and relS to binary files */
    char relS_name[128];
    char relR_name[128];
    ...
    fprintf(stdout, "[INFO ] Saving relation R to file '%s' : ", relR_name);
    fflush(stdout);
    save_relation_to_binary_file(&relR, relR_name);
    printf("OK \n");

    fprintf(stdout, "[INFO ] Saving relation S to file '%s' : ", relS_name);
    fflush(stdout);
    save_relation_to_binary_file(&relS, relS_name);
    printf("OK \n");

    printf("[INFO ] Data dumped to binary files. Exiting.\n");
    return 0;
#endif /* DUMP */

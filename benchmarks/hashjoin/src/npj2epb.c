...
#ifdef ENABLE_BUCKET_STATS
#define INIT_BUCKET_STATS() \
    uint64_t total_buckets_probed = 0; \
    uint64_t buckets_with_count_1 = 0; \
    uint64_t buckets_with_count_2 = 0; \
    uint64_t buckets_with_count_more = 0;

#define UPDATE_BUCKET_STATS(b) \
    total_buckets_probed++; \
    if ((b)->count == 1) { \
        buckets_with_count_1++; \
    } else if ((b)->count == 2) { \
        buckets_with_count_2++; \
    } else if ((b)->count > 2) { \
        buckets_with_count_more++; \
    }

#define PRINT_BUCKET_STATS() \
       printf("Total buckets probed: %d\n", total_buckets_probed);
    printf("Buckets with count 1: %d (%.2f%%)\n", buckets_with_count_1,
           (buckets_with_count_1 * 100.0) / total_buckets_probed);
    printf("Buckets with count 2: %d (%.2f%%)\n", buckets_with_count_2,
           (buckets_with_count_2 * 100.0) / total_buckets_probed);
    printf("Buckets with count >2: %d (%.2f%%)\n", buckets_with_count_more,
           (buckets_with_count_more * 100.0) / total_buckets_probed);
#else
#define INIT_BUCKET_STATS()
#define UPDATE_BUCKET_STATS(b)
#define PRINT_BUCKET_STATS()
...
probe_hashtable(hashtable_t *ht, relation_t *rel)
{
    ...
    INIT_BUCKET_STATS();  // Initialize statistics variables if enabled
    ...
    for (i = 0; i < rel->num_tuples; i++)
    {
        ...
        do {
            UPDATE_BUCKET_STATS(b);  // Update statistics if enabled
    ...
    PRINT_BUCKET_STATS();  // Print statistics if enabled

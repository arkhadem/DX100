...
pvector<WeightT> DeltaStep(const WGraph &g, NodeID source, WeightT delta,
    ...
    dist[source] = 0;
#ifdef MAA1
    pvector<WeightT> all_dist_local[4];
    vector<NodeID> all_changed_nodes[4];
    vector<NodeID> changed_nodes_global;
#pragma omp parallel for
    for (int i = 0; i < 4; i++) {
        all_dist_local[i] = pvector<WeightT>(g.num_nodes(), kDistInf);
        all_dist_local[i][source] = 0;
    }
#endif
...
#pragma omp parallel
    {
#ifdef MAA1
        pvector<WeightT> &dist_local = all_dist_local[omp_get_thread_num()];
        vector<NodeID> &changed_nodes = all_changed_nodes[omp_get_thread_num()];
#elif defined(MAA)
        pvector<WeightT> dist_local(g.num_nodes(), kDistInf);
        dist_local[source] = 0;
#endif
        t.Start();
            ...
            for (size_t i = 0; i < curr_frontier_tail; i++) {
                ...
                if (dist_u >= delta * static_cast<WeightT>(curr_bin_index)) {
                    for (int j = VertexOffsets[u]; j < VertexOffsets[u + 1]; j++) {
                        ...
                        NodeID v = wn.v;
#if defined(MAA)
                        WeightT old_dist = dist_local[v];
                        WeightT new_dist = dist_u + wn.w;
                        if (new_dist < old_dist) {
                            changed_nodes.push_back(v);
                            dist_local[v] = new_dist;
                            // printf("OMP[%d] distance[%d] = %d = %d\n", omp_get_thread_num(), v, new_dist, all_dist_local[omp_get_thread_num()][v]);
                        }
#elif defined(MAA1)
                        WeightT old_dist = dist_local[v];
                        WeightT new_dist = dist_u + wn.w;
...
#if defined(MAA)
#pragma omp barrier
            for (NodeID i : changed_nodes) {
                WeightT old_dist = dist[i];
                WeightT new_dist = dist_local[i];
                while (new_dist < old_dist) {
                    if (compare_and_swap(dist[i], old_dist, new_dist)) {
                        size_t dest_bin = new_dist / delta;
                        if (dest_bin >= local_bins.size())
                            local_bins.resize(dest_bin + 1);
                    ...
                    old_dist = dist[i]; // swap failed, recheck dist update & retry
                }
            }
#elif defined(MAA1)
            t.Stop();
...
#pragma omp single
            {
                changed_nodes_global.clear();
                for (int i = 0; i < 4; i++)
                    changed_nodes_global.insert(changed_nodes_global.end(), all_changed_nodes[i].begin(), all_changed_nodes[i].end());
                sort(changed_nodes_global.begin(), changed_nodes_global.end());
                changed_nodes_global.erase(unique(changed_nodes_global.begin(), changed_nodes_global.end()), changed_nodes_global.end());
            }
#pragma omp for nowait schedule(dynamic, 64)
            // for (NodeID i = 0; i < g.num_nodes(); i++) {
            for (NodeID i : changed_nodes_global) {
                // printf("Node[%d] = %d\n", i, all_dist_local[0][i]);
                WeightT old_dist = dist[i];
                WeightT new_dist = old_dist;
                for (int j = 0; j < 4; j++) {
                    new_dist = min(new_dist, all_dist_local[j][i]);
                }
                if (new_dist < old_dist) {
                    dist[i] = new_dist;
                    size_t dest_bin = new_dist / delta;

...
protected:
    void setup(IFrontEnd *frontend, IMemorySystem *memory_system) {
        ...
        m_channel_count = count[m_dram->m_levels("channel")];
...
public:
    ...
    void setup(IFrontEnd *frontend, IMemorySystem *memory_system) override {
        ...
        m_addr_bits[0] += calc_log2(m_system_count);
    }
    ...
    void apply(Request &req) override {
        ...
        req.addr_vec[0] %= m_channel_count;
    }
};
...
public:
    ...
    void setup(IFrontEnd *frontend, IMemorySystem *memory_system) override {
        LinearMapperBase::setup(frontend, memory_system);
        m_addr_bits[0] += calc_log2(m_system_count);
        printf("Address mapper: there is %d system, %d channels, %d channel bits\n",
               m_system_count, m_channel_count, m_addr_bits[0]);
    }

    void apply(Request &req) override {
        req.addr_vec.resize(m_num_levels, -1);
        Addr_t addr = req.addr >> m_tx_offset;
        req.addr_vec[0] = slice_lower_bits(addr, m_addr_bits[0]) % m_channel_count;

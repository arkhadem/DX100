...
public:
    void init() override {
        ...
        int bubble_count = param<int>("bubble_count").desc("Bubble Count.").required();

        // LLC params
        int llc_latency = param<int>("llc_latency").desc("Aggregated latency of the LLC.").default_val(42);
        ...
        for (int id = 0; id < m_num_cores; id++) {
            SimpleO3Core *core = new SimpleO3Core(id, ipc, depth, trace_list[id], m_translation, m_llc, bubble_count);
